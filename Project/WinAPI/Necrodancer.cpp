...
void Necrodancer::render(void)
{
	...
	if (hp != hpMax)
		...
		{
			if(count >= 1)
				IMAGEMANAGER->findImage(KEY_UI_MONSTER_HEART_FULL)->render(getMemDC(), renderPos.x - (24 * hpMax / 2) + i * 24, renderPos.y - 102);
			else
				IMAGEMANAGER->findImage(KEY_UI_MONSTER_HEART_EMPTY)->render(getMemDC(), renderPos.x - (24 * hpMax / 2) + i * 24, renderPos.y - 102);
